  ...
  <div class="h-full flex flex-col p-5 overflow-hidden" :class="isDark ? 'card-dark' : 'card-light'">
    ...
    <div class="flex justify-between items-center mb-4 pb-4 border-b" 
      ...
      <div class="text-right">
         <span class="block text-xl font-bold font-mono text-rose-500">{{ format(totalDebit) }}</span>
         <input type="month" v-model="store.currentMonth" 
               class="bg-transparent border-none text-xs font-mono cursor-pointer focus:outline-none text-right opacity-70 hover:opacity-100"
    ...
    <div class="flex-1 overflow-y-auto custom-scroll">
      <table class="w-full text-sm text-left">
        ...
        <tbody>
          <tr v-for="t in debitList" :key="t.id" 
              class="group border-b last:border-0 transition"
             ...
             <td class="py-3 text-right">
               <button @click="store.deleteItem('transaction', t.id)" class="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 px-2 transition">×</button>
             </td>
          </tr>
        </tbody>
      </table>
