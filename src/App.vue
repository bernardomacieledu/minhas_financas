...
// Inicializa a store (carrega do LocalStorage)
onMounted(() => {
  store.init()
})
...
// Handlers de Envio (Garantindo conversão para Number)
const submitAsset = () => {
...
const submitCard = () => {
  store.addCard({ ...newCard.value, currentInvoice: Number(newCard.value.currentInvoice) })
  ...
  <div class="min-h-screen bg-gray-900 text-white font-sans selection:bg-neon selection:text-black p-4 md:p-8">
    ...
    <header class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
      ...
      <div class="flex gap-2">
        <button @click="store.exportJSON" class="btn-secondary">Exportar JSON</button>
        <button @click="showImportModal = true" class="btn-secondary">Importar</button>
      </div>
    </header>
    ...
    <main class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      <div class="glass-panel col-span-1 md:col-span-1 p-4 flex flex-col">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-neon font-bold flex items-center gap-2">
            <span>💰</span> Ativos / Renda
          </h3>
        ...
        <div class="space-y-2 mb-4 flex-1 overflow-y-auto custom-scroll max-h-40">
          ...
          <div v-if="store.assets.length === 0" class="text-xs text-gray-600 text-center py-2">Nenhum ativo registrado</div>
        </div>
        ...
        <form @submit.prevent="submitAsset" class="flex gap-2 mt-auto">
          ...
          <input v-model="newAsset.value" type="number" step="0.01" placeholder="R$ 0,00" class="input-glass w-24" required>
      ...
      <div class="glass-panel col-span-1 md:col-span-1 p-4 flex flex-col">
        ...
        <form @submit.prevent="submitCard" class="flex gap-2 mt-auto">
          ...
          <input v-model="newCard.currentInvoice" type="number" step="0.01" placeholder="Fatura Atual" class="input-glass w-28">
          <button class="btn-primary">+</button>
        </form>
      </div>
      ...
      <div class="glass-panel col-span-1 md:col-span-2 p-4">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-orange-400 font-bold flex items-center gap-2">
            <span>🤝</span> A Receber (Devedores)
          </h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4 max-h-48 overflow-y-auto custom-scroll">
           <div v-for="item in store.receivables" :key="item.id" class="bg-white/5 p-3 rounded-lg border border-white/5 flex justify-between items-center hover:border-orange-400/30 transition-colors">
              ...
              <div class="flex flex-col items-end gap-1">
                ...
                <button @click="store.deleteItem('receivable', item.id)" 
                        class="text-[10px] text-danger border border-danger/50 px-2 py-0.5 rounded hover:bg-danger hover:text-white transition-colors">
                  Recebido
                </button>
              </div>
           </div>
           <div v-if="store.receivables.length === 0" class="col-span-full text-center text-xs text-gray-600 py-4">
             Ninguém te deve nada (ainda bem!)
           </div>
        </div>
        ...
        <form @submit.prevent="submitReceivable" class="flex flex-col md:flex-row gap-2 items-end border-t border-glassBorder pt-4">
          <div class="flex-1 w-full space-y-2">
             ...
             <input v-model="newReceivable.desc" placeholder="Motivo (ex: Parcela Iphone)" class="input-glass w-full text-xs">
          </div>
          <button class="btn-primary w-full md:w-auto h-10 md:h-auto whitespace-nowrap px-6">Adicionar</button>
        </form>
      </div>

       <div class="glass-panel col-span-1 p-4 flex flex-col">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-danger font-bold flex items-center gap-2">
            <span>📉</span> Fixos (Luz/Água)
          </h3>
      ...
      <div class="glass-panel col-span-1 md:col-span-3 lg:col-span-2 p-6 border border-neon/20 shadow-[0_0_15px_rgba(0,243,255,0.05)]">
        ...
        <form @submit.prevent="submitTransaction" class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-6">
          <div class="md:col-span-4">
            <input v-model="newTransaction.desc" placeholder="Ex: Almoço, Uber" class="input-glass w-full h-12" required>
          </div>
          
          <div class="md:col-span-3">
             <input v-model="newTransaction.value" type="number" step="0.01" placeholder="Valor R$" class="input-glass w-full h-12 font-mono" required>
          </div>
          ...
          <div class="md:col-span-2">
            <button class="btn-primary w-full h-12 font-bold uppercase tracking-widest text-xs shadow-lg shadow-neon/20">
              Salvar
            </button>
          </div>
        ...
        <div class="border-t border-glassBorder pt-4">
          <h4 class="text-[10px] text-gray-500 uppercase font-bold mb-3 tracking-widest">Últimos Registros</h4>
          <div class="flex flex-col gap-2">
            <div v-for="t in store.transactions.slice().reverse().slice(0, 4)" :key="t.id" class="flex justify-between items-center text-sm p-3 bg-glass rounded-lg border border-transparent hover:border-glassBorder transition-colors">
              <div class="flex items-center gap-3">
                 ...
                 <span class="font-medium text-gray-200">{{ t.desc }}</span>
                 <span class="text-xs text-gray-500" v-if="t.type === 'credit'">
                    (no cartão)
                 </span>
              </div>
              <div class="flex items-center gap-3">
                 <span class="text-danger font-mono font-bold">- {{ formatMoney(t.value) }}</span>
                 <button @click="store.deleteItem('transaction', t.id)" class="text-gray-600 hover:text-danger text-xs px-2 py-1 rounded hover:bg-white/5 transition">x</button>
              </div>
            </div>
             <div v-if="store.transactions.length === 0" class="text-sm text-gray-600 italic">Nenhum gasto registrado hoje.</div>
          </div>
        </div>
      </div>
    ...
    <div v-if="showImportModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
      <div class="glass-panel p-6 w-full max-w-lg shadow-2xl shadow-neon/10">
        <h3 class="mb-4 font-bold text-xl text-neon">Importar Dados JSON</h3>
        <p class="text-xs text-gray-400 mb-2">Cole o conteúdo do arquivo JSON gerado anteriormente.</p>
        <textarea v-model="importText" class="input-glass w-full h-48 text-xs font-mono mb-4 resize-none focus:border-neon" placeholder='{"assets": []...}'></textarea>
        <div class="flex justify-end gap-3">
          <button @click="showImportModal = false" class="px-4 py-2 rounded hover:bg-white/10 text-gray-300 text-sm transition">Cancelar</button>
          <button @click="handleImport" class="btn-primary text-sm px-6">Carregar Dados</button>
        </div>
      </div>
    </div>
  </div>
...
/* NOTA: O @tailwind e o body { bg } devem estar no style.css, não aqui */
